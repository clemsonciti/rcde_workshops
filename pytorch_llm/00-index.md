...
This workshop focuses on developing an understanding of the fundamentals of attention and transformer architecture. Participants will understand LLMs work and apply them to their own projects.
...
This copies a script `download.sh` that, when run, will copy the full workshop files to your drivespace. 
Now that you have that script, run it using the command: `bash download.sh`. You should now have a folder, `pytorch_llm`, which contains this notebook and the rest of the workshop.

## Creating a conda environment
In order to run the notebooks in this workshop, you will need a python environment in which you have installed the libraries we will be using. The below instructions will walk you through the process of creating such an environment. For our documentation regarding creating python environments, see the [Adding New Notebooks page](https://docs.rcd.clemson.edu/openod/apps/jupyter/#adding-new-notebook-kernels).
* Step 1: ssh into the cluster (see instructions [for Connecting to Palmetto through SSH](https://docs.rcd.clemson.edu/palmetto/connect/ssh/)).
...
Now you’re ready to visit Open OnDemand and request a Jupyter session using your new `AttentionWorkshop` environment.
